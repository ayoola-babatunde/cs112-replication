#Notes from replication
#links to open files should be adjusted to correct location. 
#The files used here can be found at the Harvard Dataverse
#site for this paper (https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/24896)
#Note: a new file called newdmatched is created to represent 
#'datamatched.tab' from Harvard Dataverse except using GenMatch

datamatch <- get(load("C:/Users/Ayoola_PC/Documents/datamatch.Rdata"))
...
#__________________________ Matching (with GenMatch) ________________________#
...
X = cbind(datamatch$polling.place, datamatch$age.group, datamatch$educ, 
          datamatch$male, datamatch$tech, datamatch$pol.info, datamatch$white.collar, datamatch$not.full.time,
          (datamatch$age.group)^2, (datamatch$age.group)^3, (datamatch$educ)^2, (datamatch$tech)^2)



#The covariates we want to obtain balance on
BalanceMat <- cbind(X, I(datamatch$age.group*datamatch$educ), I(datamatch$age.group*datamatch$tech),  
                    I(datamatch$educ*datamatch$pol.info),I(datamatch$age.group*datamatch$pol.info), 
                    I(datamatch$tech*datamatch$pol.info) 
)

genout <- GenMatch(Tr=datamatch$EV, X=X, BalanceMatrix=BalanceMat, estimand="ATT", M=1, ties = F,
                   pop.size=200, max.generations=100, wait.generations=4)

#The outcome variable
Y=datamatch$eval.voting


mout <- Match(Y=Y, Tr=datamatch$EV, X=X, estimand="ATT", Weight.matrix=genout)
summary(mout)

#                        
#Let's determine if balance has actually been obtained on the variables of interest
#                        
mb <- MatchBalance(EV ~ age.group + educ + white.collar + not.full.time + 
                     male + tech + pol.info, data = datamatch, match.out=mout, nboots=500)


##########################################################################################################################
#constructing the new datamatcheed
newdmatched <- rbind(datamatch[mout$index.treated,], datamatch[mout$index.control,])
